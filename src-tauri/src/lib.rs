...
#[tauri::command]
fn compare_image_folders(left_path: String, right_path: String) -> Result<Vec<CompareResult>, String> {
...
fn load_image_preview(path: String) -> Result<ImagePreview, String> {
  ...
  let mime_type = mime_type_for_path(&image_path)
    .ok_or_else(|| "当前图片格式不支持预览".to_string())?;
...
mod tests {
  ...
  fn write_png(path: &Path, width: u32, height: u32) {
    let image = ImageBuffer::<Rgba<u8>, Vec<u8>>::from_pixel(width, height, Rgba([0, 0, 0, 255]));
  ...
  fn reports_size_mismatch_and_missing_files() {
    ...
    assert!(results.iter().any(|item| item.file_name == "diff.png" && item.status == CompareStatus::SizeMismatch));
    assert!(results.iter().any(|item| item.file_name == "left-only.png" && item.status == CompareStatus::MissingRight));
    assert!(results.iter().any(|item| item.file_name == "right-only.png" && item.status == CompareStatus::MissingLeft));
  }
